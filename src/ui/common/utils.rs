...
pub fn apply_horizontal_scroll(line: Line<'static>, horizontal_offset: usize, viewport_width: usize) -> Line<'static> {
...
pub fn create_commits_title(commits_count: usize, loading: bool, horizontal_scroll: usize) -> String {
    ...
    let mut title = if commits.is_empty() {
        ...
        if let Some((older_idx, newer_idx)) = current_diff_range {
            if older_idx < commits.len() && newer_idx < commits.len() {
                format!(" Diff ({}..{}) ", 
                    commits[older_idx].short_hash,
                    commits[newer_idx].short_hash)
            } else {
            ...
            format!(" Diff ({}) [Selecting...] ", commits[selected_index].short_hash)
        } else {
            format!(" Diff ({}) ", commits[selected_index].short_hash)
        }
    } else {
...
pub fn create_side_by_side_title(
    ...
    if commits.is_empty() {
        return if is_old_file { " Old File ".to_string() } else { " New File ".to_string() };
    }
