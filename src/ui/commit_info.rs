...
impl CommitInfoPopup {
    ...
    fn render_metadata(&self, frame: &mut Frame, area: Rect) {
        ...
        lines.push(Line::from(vec![
            Span::styled("Hash:      ", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)),
            Span::styled(&self.commit.hash, Style::default().fg(Color::Cyan)),
        ]));
        
        // Author
        lines.push(Line::from(vec![
            Span::styled("Author:    ", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)),
        ...
        if self.commit.committer_name != self.commit.author_name || 
           self.commit.committer_email != self.commit.author_email {
            lines.push(Line::from(vec![
                Span::styled("Committer: ", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)),
                Span::raw(format!("{} <{}>", self.commit.committer_name, self.commit.committer_email)),
            ]));
            
            if !self.commit.committer_date.is_empty() && self.commit.committer_date != self.commit.author_date {
                lines.push(Line::from(vec![
                    Span::styled("Commit Date:", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)),
        ...
        if !self.commit.refs.is_empty() {
            ...
            lines.push(Line::from(vec![
                Span::styled("Refs:      ", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)),
                Span::styled(format!("({})", refs_text), Style::default().fg(Color::Magenta)),
            ]));
        }
        ...
        if let Some(ref pr_info) = self.commit.pr_info {
            lines.push(Line::from(vec![
                Span::styled("PR:        ", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)),
                Span::styled(format!("#{} - {}", pr_info.number, pr_info.title), Style::default().fg(Color::Green)),
            ]));
        }
        ...
        if let Some(ref stats) = self.commit.stats {
            lines.push(Line::from(vec![
                ...
                Span::styled(
                    format!("{} files, +{} -{} lines", stats.files_changed, stats.insertions, stats.deletions),
    ...
    fn render_message(&self, frame: &mut Frame, area: Rect) {
        ...
        if !self.commit.body.is_empty() {
            ...
            message_lines.push(Line::from(vec![
                Span::styled("(No additional message content)", Style::default().fg(Color::DarkGray))
            ]));
        }
